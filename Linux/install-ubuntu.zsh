...
if ! command_exists snap; then
  ...
  sudo sudo apt-get install snapd
else
...
echo "To finish setup you need download Meslo Nerd Font :
...
if [ -z "$ZSH_CUSTOM" ]; then
  ...
  echo "Setting ZSH_CUSTOM to default path: $ZSH_CUSTOM"
fi

# Ensure write permissions on $ZSH_CUSTOM/themes
chmod -R u+w "$ZSH_CUSTOM/themes"
...
if [ ! -d "$ZSH_CUSTOM/themes/powerlevel10" ]; then
...
if ! command_exists lsd; then
  ...
  sudo dpkg -i lsd-musl_1.1.2_amd64.deb && rm -f lsd-musl_1.1.2_amd64.deb
else
  echo "lsd is already installed."
fi

# Configure powerlevel10k in ~/.zshrc if not already configured
if ! grep -q 'powerlevel10k' "$HOME/.zshrc"; then
  echo "Configuring powerlevel10k in ~/.zshrc..."
  echo 'source "$ZSH_CUSTOM/themes/powerlevel10/powerlevel10k.zsh-theme"' >> ~/.zshrc
else
  echo "powerlevel10k is already configured in ~/.zshrc."
fi

if ! grep -q 'zsh-syntax-highlighting' "$HOME/.zshrc"; then
  echo "Configuring zsh-syntax-highlighting in ~/.zshrc..."
  echo "" >> ~/.zshrc  # Ajout d'un saut de ligne
  echo 'source "$ZSH_CUSTOM/plugins/zsh-syntax-highlighting/zsh-syntax-highlighting.zsh"' >> ~/.zshrc
else
  echo "zsh-syntax-highlighting is already configured in ~/.zshrc."
fi

if ! grep -q 'zsh-completions' "$HOME/.zshrc"; then
  echo "Configuring zsh-completions in ~/.zshrc..."
  echo "" >> ~/.zshrc  # Ajout d'un saut de ligne
  echo 'source "$ZSH_CUSTOM/plugins/zsh-completions/zsh-completions.plugin.zsh"' >> ~/.zshrc
else
  echo "zsh-completions is already configured in ~/.zshrc."
fi
...
# Ajouter lsd au PATH et définir un alias ls
if command_exists lsd && ! grep -q 'alias ls="lsd"' "$HOME/.zshrc"; then
  echo "Configuring lsd in ~/.zshrc..."
  echo "" >> ~/.zshrc  # Ajout d'un saut de ligne
